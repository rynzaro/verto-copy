                    ...
                    <div className="w-[360px]">
                        <div className="border border-gray-600 rounded-md divide-y divide-gray-600 w-full my-2">
                            <div className="flex justify-between px-4 py-3 font-bold text-sm">
                                ...
                                <div>{tokenObjects[order.from_contract_id].symbol} - {tokenObjects[order.to_contract_id].symbol}</div>
                            </div>
                            <div className="flex justify-between px-4 py-3 font-bold text-sm">
                            ...
                            <div className="flex justify-between px-4 py-3 font-bold text-sm">
                                ...
                                <div>{formatNumber(Number(convertIntToFloat(order.to_amount, tokenObjects[order.to_contract_id].decimals)))}</div>
                            </div>
                            <div className="flex justify-between px-4 py-3 font-bold text-sm">
                                <div className="text-gray-400">You receive</div>
                                <div>{formatNumber(Number(convertIntToFloat(order.from_amount, tokenObjects[order.from_contract_id].decimals)))}</div>
                            </div>
                            <div className="flex justify-between px-4 py-3 font-bold text-sm">
                                <div className="text-gray-400">Price Per Token</div>
                                <div>{formatNumber(
                                    parseFloat(convertIntToFloat(order.from_amount, tokenObjects[order.from_contract_id].decimals)) /
                                    parseFloat(convertIntToFloat(order.to_amount, tokenObjects[order.to_contract_id].decimals))
                                )}</div>
                            </div>
                            <div className="flex justify-between px-4 py-3 font-bold text-sm">
